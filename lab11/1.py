...
def insert_or_update_user(conn, first_name, last_name, phone_number):
    try:
        cur = conn.cursor()
        cur.execute("""SELECT id FROM Phonebook WHERE first_name = %s AND last_name = %s""", (first_name, last_name))
...
def insert_users_bulk(conn, users_data):
    try:
        ...
        for user_data in users_data:
            if len(user_data) == 3 and len(user_data[2]) == 11:  # Assuming phone number length should be 11
                insert_data(conn, user_data[0], user_data[1], user_data[2])
            else:
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                ...
                last_name = input("Enter last name: ")
                phone_number = input("Enter phone number: ")
                users_data.append((first_name, last_name, phone_number))
